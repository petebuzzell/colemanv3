...
#
# Commands:
#   hubot mixtape add <query> - Adds first track in search to the playlist
#   hubot mixtape addid <track_id> - Adds a track to the playlist via ID
#   hubot mixtape remove <track_id>  - Removes a track on the playlist
#   hubot mixtape find <query> - go to that link to get a token
#
# Author:
...
module.exports = (robot) ->
  ...
  robot.hear /mixtape add (.*)/i, (res) ->
    authorizeApp(res, findAndAddFirstTrack)

  robot.hear /mixtape addid (.*)/i, (res) ->
    authorizeAppUser(res, addTrack)

  robot.hear /mixtape remove (.*)/i, (res) ->
    authorizeAppUser(res, removeTrack)

  robot.hear /mixtape find (.*)/i, (res) ->
